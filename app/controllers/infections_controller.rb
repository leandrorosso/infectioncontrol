class InfectionsController < ApplicationController
  before_action :authorize_infection, only: [:show, :edit, :update, :destroy] 
  before_action :set_infection, only: %i[ show edit update destroy ]

  # GET /infections or /infections.json
  def index
    @infections = policy_scope(Infection).all
  end
  ...
  private
    ...
    def infection_params
      params.require(:infection).permit(:description, :active, :bacterium_id).merge(user_id: current_user.id)
    end
    
    def authorize_infection
      authorize @infection
    end
end
